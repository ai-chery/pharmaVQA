...
class BANLayer(nn.Module):
    ...
    def forward(self, v, q, v_mask=None, q_mask=None, softmax=False):
        v_num = v.size(1)
        q_num = q.size(1)
        ...
        if softmax:
            # masking
            atten_mask = []
            for i in range(v_mask.size()[0]):
                atten_mask.append(torch.matmul(v_mask[i].reshape(-1, 1), q_mask[i].reshape(1, -1).float()))
            atten_mask = torch.stack(atten_mask)
            atten_mask = atten_mask.unsqueeze(1).repeat(1, self.h_out, 1, 1)
