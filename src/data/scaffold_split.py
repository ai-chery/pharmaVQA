...
from splitters import random_split, random_scaffold_split, scaffold_split

root_path = '../../datasets/vs'
dataset = 'HPK1_KI'
use_split_method = 'seed_scaffold_split'
seeds = [0, 1, 2]
MPG_seeds = [88, 89, 90]
try:
...
for seed in seeds:
    if use_split_method == 'use_recommend_split':
        ...
        for seed in seeds:
            ...
            train_index, valid_index, test_index = scaffold_split.split(
                dataset=smiles_list, all_index=torch.arange(len(smiles_list)), frac_train=0.8, frac_valid=0.1,
                frac_test=0.1)
            save_index = [train_index.numpy().tolist(), valid_index.numpy().tolist(), test_index.numpy().tolist()]
        ...
        smiles_list = pd.read_csv(dataset_path)['smiles'].dropna().tolist()
        print(f'{dataset} has {len(smiles_list)} smiles')
        train_index, valid_index, test_index = MoleculeSTM_scaffold_split(
            torch.arange(len(smiles_list)), smiles_list=smiles_list, null_value=0, frac_train=0.8,
    ...
    elif use_split_method == 'scaffold_split':
        smiles_list = pd.read_csv(dataset_path)['smiles'].dropna().tolist()
        print(f'{dataset} has {len(smiles_list)} smiles')
        ...
        filename = os.path.join(root_path, f"{dataset}/splits/scaffold.json")
        with open(filename, 'w') as file_obj:
            json.dump(save_index, file_obj)
    elif use_split_method == 'seed_scaffold_split':
        print(f'{dataset} has {len(smiles_list)} smiles')
        train_index, valid_index, test_index = random_scaffold_split(
            torch.arange(len(smiles_list)), smiles_list=smiles_list, null_value=0, frac_train=0.8,
            frac_valid=0.1, frac_test=0.1, seed=seed)
        ...
        filename = os.path.join(root_path, f"{dataset}/splits/scaffold-{seed}.json")
        if not os.path.exists(os.path.join(root_path, f"{dataset}/splits")):
            os.makedirs(os.path.join(root_path, f"{dataset}/splits"))
