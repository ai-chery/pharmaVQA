...
class MoleculeNetVQADataset(InMemoryDataset):
    def __init__(self, root, dataset='zinc250k', transform=None, text_tokenizer=None, question_num=1, text_max_len=256,
                 device=None,
        ...
        super(MoleculeNetVQADataset, self).__init__(root, transform, pre_transform, pre_filter)

        self.tokenizer = text_tokenizer
        self.text_max_len = text_max_len
        text_path = os.path.join('./data', 'text', 'phar_question_howmany.json')
        phar_path = os.path.join('./data', f"chem_dataset_MoMu/dataset/{dataset}/raw/phar_features.pkl")
    ...
    def get(self, idx):
        ...
        select_question = [random.sample(list(self.text_list[random_question].values()), 1)[0] for random_question in
                           random_questions]
        texts, masks = self.tokenizer_text(select_question)
        ...
        phar_targets = attn_phar_atom.sum(dim=1).long()

        str_ans = [str(ans) for ans in phar_targets]
        ans, ans_mask = self.tokenizer_text(str_ans)

        return data, (texts, masks, ans, ans_mask, phar_targets)

    def tokenizer_text(self, text):
        text_tokens_ids, text_masks = prepare_text_tokens(
            device=self.device, description=text, tokenizer=self.tokenizer, max_seq_len=self.text_max_len)
    ...
    def process(self):
        ...
        if self.dataset == 'tox21':
            ...
            data_list, data_smiles_list, data_label_list = shared_extractor(
                smiles_list, rdkit_mol_objs, labels)
        elif self.dataset == 'chembl29':
            smiles_list, rdkit_mol_objs, labels = \
                _load_pretrain_dataset(self.raw_paths[0])
            input = zip(smiles_list, rdkit_mol_objs, labels)
            n_jobs = 16
            with Pool(processes=n_jobs) as pool:
                results = list(tqdm(pool.imap(shared_extractor, input), total=len(smiles_list)))

            data_list, data_smiles_list, data_label_list = results[0], results[1], results[2]
        else:
...
class MoleculeNetGraphDataset_molformer(InMemoryDataset):
    def __init__(self, root, dataset='zinc250k', transform=None,
                 pre_transform=None, pre_filter=None, empty=False):
        ...
        super(MoleculeNetGraphDataset_molformer, self).__init__(root, transform, pre_transform, pre_filter)
    ...
    def process(self):
        ...
        if self.dataset == 'tox21':
            ...
            smiles_list, rdkit_mol_objs, labels = \
                _load_bace_dataset_molformer(self.raw_paths[0])
            data_list, data_smiles_list, data_label_list = shared_extractor(
                smiles_list, rdkit_mol_objs, labels)
...
def _load_bace_dataset_molformer(input_path):
    input_df = pd.read_csv(input_path, sep=',')
    smiles_list = input_df['smiles']
    rdkit_mol_objs_list = [AllChem.MolFromSmiles(s) for s in smiles_list]
    labels = input_df['Class']
...
def _load_moleculeace_dataset(input_path):
    ...
    assert len(smiles_list) == len(labels)
    return smiles_list, rdkit_mol_objs_list, labels.values
def _load_tdc_dataset(input_path):
...
def _load_pretrain_dataset(input_path):
    ...
    print('multi thread processing mols')
    n_jobs = 32
    with Pool(processes=n_jobs) as pool:
        rdkit_mol_objs_list = list(tqdm(pool.imap(mol_from_smiles, smiles_list), total=len(smiles_list)))
    ...
    assert len(smiles_list) == len(preprocessed_rdkit_mol_objs_list)
    assert len(smiles_list) == len(smiles_list)
    assert len(smiles_list) == len(labels)
    return list(smiles_list), \
        preprocessed_rdkit_mol_objs_list, labels
