...
from src.trainer.finetune_trainer_ace import Trainer
...
def parse_args():
    ...
    parser.add_argument("--split", type=str, default='ace')
    ...
    parser.add_argument('--phar_name', type=str, default='all',
                        help="{'Donor', 'Acceptor', 'NegIonizable', 'PosIonizable', 'Aromatic', 'Hydrophobe','LumpedHydrophobe','all'}")
...
def finetune(args):
    ...
    if args.dataset_type == 'classification':
        ...
        r2_evaluator = Evaluator(args.dataset, 'r2', train_dataset.n_tasks, mean=train_dataset.mean.numpy(),
                                 std=train_dataset.std.numpy())
    ...
    summary_writer = SummaryWriter(
        f"tensorboard/finetune-{args.dataset}/questionNum{args.num_questions}/split{args.split}/seed{args.seed}/lr{args.lr}/st{starttime}",
        comment=starttime)
    trainer = Trainer(args, optimizer, lr_scheduler, loss_fn, phar_loss_fn, align_loss_fn, evaluator, phar_evaluator,
                      result_tracker,
                      summary_writer,
                      r2_evaluator=r2_evaluator,
                      device_id=device,
                      model_name='LiGhT',
    ...
    best_train, best_val, best_test, best_train_pharNum, best_val_pharNum, best_test_pharNum, _, _, r2_result = trainer.fit(
        model,
        train_loader,
        None,
        test_loader)
    ...
    print(f"test r2: {r2_result:.3f}")
    return best_test, r2_result


if __name__ == '__main__':
    args = parse_args()
    seed = [25, 32, 33, 34, 67, 68, 69, 1, 2, 6]
    results = pd.DataFrame([], columns=['name', 'seed', 'rmse_results', 'r2_results'])
    for runs_idx in range(args.num_runs):
        # args.seed = seed[runs_idx]
        args.seed += 1
        set_random_seed(args.seed)
        rmse_results, r2_results = finetune(args)
        ...
        print(
            f'Dataset {args.dataset} on {args.split} at seed {args.seed} rmse results is: {rmse_results}, r2_results is: {r2_results}')
